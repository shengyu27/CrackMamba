...
class nnUNetTrainer(object):
    ...
    def on_validation_epoch_end(self, val_outputs: List[dict]):
        ...
        #global_iou_per_class = [i for i in [i / (i + j + k) for i, j, k in zip(tp, fp, fn)]]
    ...
    def on_epoch_end(self):
        ...
        if self._best_ema is None or self.logger.my_fantastic_logging['ema_fg_dice'][-1] > self._best_ema:
            ...
            if current_epoch <= 500:
                self.save_checkpoint(join(self.output_folder, 'checkpoint_best_500.pth'))
                self.print_to_log_file(f"Yayy! New best checkpoint is in : "+str(current_epoch)+' epoch')
    ...
    def perform_actual_validation(self, save_probabilities: bool = False):
        ...
        if self.local_rank == 0:
            ...
            self.print_to_log_file("Mean Validation IoU: ", (metrics['foreground_mean']["IoU"]), also_print_to_console=True)
